...
var input = document.querySelectorAll("#text_area")[0],
  ...
  topKeywords = document.querySelector("#topKeywords"); // updating the displayed stats after every keypress

input.addEventListener("keyup", function() {
  //keeping the console clean to make only the latest data visible
  console.clear(); // character count
  // just displaying the input length as everything is a character

  characterCount.innerHTML = input.value.replace(/[^а-шА-Ш]/g, "").length; // word count using \w metacharacter - replacing this with .* to match anything between word boundaries since it was not taking 'a' as a word.
  ...
  var words = input.value.match(/\s[\w\u0402-\u045f]+/gi);
  ...
  if (words) {
    var sentences = input.value.split(/\s[\w\u0402-\u045f]+/gi);
